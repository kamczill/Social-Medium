...
def like_post(file: UploadFile = File(...), user_id: str = Depends(oauth2.require_user)):
    ...
    User.update_one({"_id": ObjectId(str(user_id))}, { "$set": {"picturePath": path}})
...
def login(payload: schemas.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
    ...
    access_token = Authorize.create_access_token(
        subject=str(user["id"]), expires_time=timedelta(minutes=float(ACCESS_TOKEN_EXPIRES_IN)))

    # Create refresh token
    refresh_token = Authorize.create_refresh_token(
        subject=str(user["id"]), expires_time=timedelta(minutes=float(REFRESH_TOKEN_EXPIRES_IN)))

    # Store refresh and access tokens in cookie
    response.set_cookie('access_token', access_token, ACCESS_TOKEN_EXPIRES_IN * 60,
                        ACCESS_TOKEN_EXPIRES_IN * 60, '/', None, True, True, samesite='lax')
    response.set_cookie('refresh_token', refresh_token,
                        REFRESH_TOKEN_EXPIRES_IN * 60, REFRESH_TOKEN_EXPIRES_IN * 60, '/', None, True, True, 'lax')
    response.set_cookie('logged_in', 'True', ACCESS_TOKEN_EXPIRES_IN * 6000,
                        ACCESS_TOKEN_EXPIRES_IN * 60000, '/', None, True, False, 'lax')
